...
public class OrderResponseMessageFactoryImpl implements OrderMessageFactory {
    ...
    private String getSubjectText(final UserRequestStatus status) {
        switch (status) {
            case ACCEPTED: return templateStringBuilder.build(SUBJECT_TEMPLATE_PATH,
                    new HashMap<String, Object>(){{put(STATUS, getLocalizedParam(MESSAGE_TEXT_ACCEPTED));}});
            case REJECTED:  return templateStringBuilder.build(SUBJECT_TEMPLATE_PATH,
                    new HashMap<String, Object>(){{put(STATUS, getLocalizedParam(MESSAGE_TEXT_REJECTED));}});
