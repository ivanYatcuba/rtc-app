...
public class JobManager {
    ...
    public void manageJob(ReportDetails report, JobManagerAction managerAction) throws Exception {
        ...
        if(managerAction == JobManagerAction.CREATE || managerAction == JobManagerAction.UPDATE){
            ...
            scheduler.getObject().scheduleJob(newJob(reportJob.getClass()).withIdentity(report.getCode()).
                    setJobData(dataMap).build(),
                    newTrigger().withIdentity(report.getCode()).withSchedule(cronSchedule(croneExpression)).build());
